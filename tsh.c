...
void eval(char *cmdline)
{
    ...
    isBackground = parseline(cmdline, arguments);
    if (arguments[0] == NULL)
        return;
    sigset_t signalMask;
    ...
    if (!builtin_cmd(arguments)){
        ...
        while (*currentArg){
            if (!strcmp(*currentArg, "<")){
                ...
                if ((inputFileDescriptor = open(*(currentArg + 1), O_RDONLY)) < 0)
                    perror("No File For Reading");
                *currentArg = NULL;
            } else if (!strcmp(*currentArg, "|")){

                if (pipe(pipeFileDescriptor) < 0)
                    unix_error("pipe");
                *currentArg = NULL;
                pipeArg = currentArg + 1;
            } else if (!strcmp(*currentArg, ">")){

                if ((outputFileDescriptor = open(*(currentArg + 1), O_WRONLY | O_CREAT | O_TRUNC, 0666)) < 0)
                    perror("No File For Writing");
                *currentArg = NULL;
            }
            currentArg++;
        }
        ...
        if (pipeArg != NULL){
            ...
            if (secondProcessID == 0){
                ...
                if (execve(pipeArg[0], pipeArg, environ) < 0){
                    printf("%s: command not found.\n", arguments[0]);
                    exit(0);
                }
            }
        ...
        if (outputFileDescriptor > 0)
            close(outputFileDescriptor);
        if (pipeFileDescriptor[0] > 0){
            close(pipeFileDescriptor[0]);
            close(pipeFileDescriptor[1]);
        }


        addjob(jobs, processID, (isBackground ? BG : FG) , cmdline);
        ...
        } else
        {
            ...
            printf("[%d] (%d) %s", getjobpid(jobs, processID)->jid, processID, cmdline);
        }
    }
    return;
}
